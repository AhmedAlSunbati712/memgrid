...
from handoff.lib.vision_runner import (
    ...
    DEFAULT_IMAGE_ROOT,
    dispatch_animals_graphs,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate animals-only layerwise gen-vs-ident graphs.")
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--output-dir", default=str(DEFAULT_GRAPH_OUTPUT))
    ...
    parser.add_argument("--quick", action="store_true", help="max-settings=1, max-seeds=1, max-layers=2")
...
def main() -> None:
    ...
    written = dispatch_animals_graphs(
        device=args.device,
