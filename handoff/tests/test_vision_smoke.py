...
def test_corrupt_ident_probe_stacks_noise_shift_and_occlusion():
    ...
    stacked_a = corrupt_ident_probe(image, seed=99)
    stacked_b = corrupt_ident_probe(image, seed=99)
    stacked_via_mode = corrupt_image(
        image,
        mode=IDENT_CORRUPTION_MODE,
        seed=99,
        noise_std=IDENT_NOISE_STD,
        max_shift=IDENT_MAX_SHIFT,
        occlusion_frac=IDENT_OCCLUSION_FRAC,
    )

    assert stacked_a.size == image.size
    assert np.array_equal(np.asarray(stacked_a), np.asarray(stacked_b))
    assert np.array_equal(np.asarray(stacked_a), np.asarray(stacked_via_mode))
    assert not np.array_equal(np.asarray(stacked_a), np.asarray(image))
    assert not np.array_equal(np.asarray(stacked_a), np.asarray(noise_only))
    assert IDENT_DECISION_NOISE_STD == 0.01
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("noise_shift_occlusion", {"noise_std": 5.0, "max_shift": 2, "occlusion_frac": 0.5}),
