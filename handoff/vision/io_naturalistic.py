...
def _default_data_paths() -> tuple[Path, Path]:
    try:
        from handoff.lib.paths import DATA_ROOT

        return DATA_ROOT / "datasets_peterson.pkl", DATA_ROOT
    except ImportError:
        return Path("datasets_peterson.pkl"), Path(".")
...
def load_naturalistic_category(
    category: str,
    *,
    dataset_pkl: str | Path | None = None,
    image_root: str | Path | None = None,
    required_exemplars: int | None = None,
) -> NaturalisticCategory:
    default_pkl, default_root = _default_data_paths()
    pkl_path = default_pkl if dataset_pkl is None else Path(dataset_pkl)
    root = default_root if image_root is None else Path(image_root)
    payload = load_peterson_dataset(pkl_path)
