...
def run_naturalistic_baseline(
    *,
    ...
    if effective_split_mode is None:
        if category == "animals":
            effective_split_mode = "random_storage_curve"
        elif category in {"fruits", "vegetables"}:
            effective_split_mode = "balanced_exemplar_folds"
        else:
            effective_split_mode = "random_storage_curve"

    if (
        required_exemplars is None
        and category == "vegetables"
        and effective_split_mode == "balanced_exemplar_folds"
    ):
